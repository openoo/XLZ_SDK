...
XLZ_PLUGIN_EXPORT const char* XLZ_PLUGIN_CALL appload(const char* apidata, const char* pluginkey) {
    ...
    xlz::AppInfo info;
    info.SetAppName("XLZ C++ SDK 示例插件");
    info.SetAuthor("openoo");
    info.SetVersion("1.0.0");
    info.SetDescription("使用 XLZ_SDK 构建的小栗子 C++ 插件示例");
    ...
    info.AddPermissionRequest("输出日志", "示例插件输出运行日志");
    info.AddPermissionRequest("发送群消息", "收到测试命令后回复群消息");
    info.AddPermissionRequest("发送好友消息", "收到测试命令后回复好友消息");
    return info.GetData();
}

XLZ_PLUGIN_EXPORT const char* XLZ_PLUGIN_CALL apprun(const char* apidata, const char* pluginkey) { return appload(apidata, pluginkey); }
