...
示例插件的初始化返回格式已经按小白兔可加载插件的格式处理：入口只导出 `appload`，权限申请的 `needapilist` 每项只包含 `desc` 字段，并且返回给宿主前转换为 ANSI/GBK。不要随意向该对象追加非框架文档字段，也不要在老宿主上申请新版本全量权限，否则部分宿主会提示“初始化函数返回值格式错误”。
